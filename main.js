...
// Control variables - declare before using in loadModel
let size = 128;
let threshold = 0.25;
let scale = 0.5;
...
async function loadModel(model_id) {
    try {
        status.textContent = `Loading ${model_id}...`;
        loadModelBtn.disabled = true;
        loadModelBtn.textContent = 'Loading...';
        
        // Reset model loaded state while loading
        isModelLoaded = false;
        
        // Load model and processor with error suppression for validation warnings
        model = await AutoModel.from_pretrained(model_id, {
            // Suppress some validation errors
            device: 'webgpu',
        }).catch(async (err) => {
            // Fallback to default device if webgpu fails
            console.warn('WebGPU not available, falling back to WASM');
            return await AutoModel.from_pretrained(model_id);
        });
        
        processor = await AutoProcessor.from_pretrained(model_id);
        
        // Set up processor size
